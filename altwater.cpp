...
void bfs(int x,int y)
{
    ...
    while(!Q.empty())
    {
       ...
       // cout<<"bool value of its neighbour "<<uflag<<" "<<rflag<<" "<<dflag<<" "<<lflag<<endl;
        int tempmin=minimum;
        bool repeatflag=0;
        if(a[prev.first][prev.second]==a[p.first][p.second])
            repeatflag=1;
        if(!lflag && a[l.first][l.second]<minimum && !vis[l.first][l.second])
            minimum=a[l.first][l.second];
        if(!rflag && a[r.first][r.second]<minimum && !vis[r.first][r.second])
            minimum=a[r.first][r.second];;
        if(!dflag && a[d.first][d.second]<minimum  && !vis[d.first][d.second])
            minimum=a[d.first][d.second];;
        if(!uflag && a[u.first][u.second]<minimum  && !vis[u.first][u.second])
